...
load ChemistryDatabase_new.mat    % --- New data base 
% load LatentHeat;
...
% load CpVapor;
%% ------------------------------------------------------------------------
HeatingRate = 40; % 20 degree/min is converted to degree/second 
%% ------------------------------------------------------------------------
...
%% Here create the cell variables 
    ...
    HF        = createCellVariable(m,interp1q(Heat_Flow(:,1)+273,Heat_Flow(:,2),Temp.value(1)));% Temperature values are converted to Kelvin. and mW is converted to W.
...
%     BCphiV.right.a = 1; BCphiV.right.b = 0; BCphiV.right.c = dphi_vaporx_E;
    ...
    for i = 1:nx+2
        ...
        HFi(i)        = interp1q(Heat_Flow(:,1)+273,Heat_Flow(:,2),Temp.value(i));% Temperature values are converted to Kelvin. and mW is converted to W.
    ...
    HF.value        = HFi';% Temperature values are converted to Kelvin. and mW is converted to W.
    kgyp.value      = kgypi';
    LHeat.value     = LHeati';
